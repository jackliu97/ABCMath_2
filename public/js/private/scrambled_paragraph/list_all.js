( function($) {
	...
	$(document).ready(function() {
		var oTable = $('#list_table').dataTable({
			...
			"bServerSide": true,
			"bLengthChange": true,
			"sAjaxSource": '/scrambled_paragraph/list_action',
		...
		$('#list_table').on( 'click', '.edit_mode', function () {
			window.location = '/scrambled_paragraph/create_from_question/' + $(this).attr('paragraph_id');
		});
		...
		$('#list_table').on( 'click', '.remove', function () {
			...
			$.ajax({
				...
				data: {'q_id':$(this).attr('paragraph_id')},
		...
		$('#list_table').on( 'click', '.more_info', function () {
			...
			$.ajax({
				type:'POST',
				url:'/scrambled_paragraph/get_paragraph_detail',
